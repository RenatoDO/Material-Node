...
#include "Materials/MaterialExpressionSpectrum.h"
...
//This is my change
UMaterialExpressionSpectrum::UMaterialExpressionSpectrum(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	// Structure to hold one-time initialization
	struct FConstructorStatics
...
#if WITH_EDITOR
int32 UMaterialExpressionSpectrum::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	int32 WInput;
	/*if (!Wavelength.GetTracedInput().Expression)
	{
		return Compiler->Errorf(TEXT("Missing Spectrum input"));
	}*/
	if (Wavelength.GetTracedInput().Expression)
	{
		WInput = Wavelength.Compile(Compiler);
	}
	else
	{
		WInput = Compiler->WorldPosition(WPT_Default);
	}

	return Compiler->Spectrum(WInput, Spectrum_Type);
	//return 0;
}

void UMaterialExpressionSpectrum::GetCaption(TArray<FString>& OutCaptions) const
{
	switch (Spectrum_Type)
	{
	case ESpectrumType::Jet:
		OutCaptions.Add(TEXT("Jet")); break;
	case ESpectrumType::Bruton:
		OutCaptions.Add(TEXT("Bruton")); break;
	case ESpectrumType::GPU_Gems:
		OutCaptions.Add(TEXT("GPU Gems")); break;
	case ESpectrumType::Spektre:
		OutCaptions.Add(TEXT("Spektre")); break;
	case ESpectrumType::Zucconi:
		OutCaptions.Add(TEXT("Zucconi")); break;
	case ESpectrumType::Zucconi6:
		OutCaptions.Add(TEXT("Zucconi6")); break;
	case ESpectrumType::Visible:
		OutCaptions.Add(TEXT("Visible")); break;
	}
	OutCaptions.Add(TEXT("Spectrum"));
}
#endif 
